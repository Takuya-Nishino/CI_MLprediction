...
# %% [markdown]
# # ① Main analysis — BMC MIDM / Methods-aligned / Standalone FULL
#
# Runs the four primary prediction tasks (Day 3/Day 7 × non-home discharge/LOS >21 days), chronological temporal evaluation, OOF threshold derivation, bootstrap CIs, and saves locked models/predictions. No external .py file is required.
#
...
def build_day7_matrix(
    ...
    nonhome = primary_results["day7_nonhome"]
    los21 = primary_results["day7_los21"]
    ...
    a = a.rename(columns={f"{model_key}_probability": "p_nonhome"})
    b = los21["temporal_predictions"][[config.id_col, f"{model_key}_probability"]].copy()
    b = b.rename(columns={f"{model_key}_probability": "p_los21"})
    matrix = a.merge(b, on=config.id_col, how="inner", validate="one_to_one")

    th_nonhome = nonhome["models"][model_key]["threshold_info"]["threshold"]
    th_los = los21["models"][model_key]["threshold_info"]["threshold"]
    matrix["high_nonhome"] = matrix["p_nonhome"].ge(th_nonhome).astype("int8")
    matrix["high_los21"] = matrix["p_los21"].ge(th_los).astype("int8")
    matrix["matrix_group"] = np.select(
        [
            matrix["high_nonhome"].eq(0) & matrix["high_los21"].eq(0),
            matrix["high_nonhome"].eq(1) & matrix["high_los21"].eq(0),
            matrix["high_nonhome"].eq(0) & matrix["high_los21"].eq(1),
            matrix["high_nonhome"].eq(1) & matrix["high_los21"].eq(1),
        ],
        [
    ...
    matrix["observed_pattern"] = np.select(
        [
            matrix["Outcome_nonhome"].eq(0) & matrix["LOS_21"].eq(0),
            matrix["Outcome_nonhome"].eq(1) & matrix["LOS_21"].eq(0),
            matrix["Outcome_nonhome"].eq(0) & matrix["LOS_21"].eq(1),
            matrix["Outcome_nonhome"].eq(1) & matrix["LOS_21"].eq(1),
        ],
        [
            "Neither outcome",
    ...
    summary = (
        matrix.groupby("matrix_group", observed=True)
        .agg(
            ...
            q1_LOS=(config.los_col, lambda x: x.quantile(0.25)),
            q3_LOS=(config.los_col, lambda x: x.quantile(0.75)),
    ...
    summary["cohort_percent"] = summary["n"] / len(matrix)
...
# %% 
# =============================================================================
# 9B. BMC MIDM publication-size figure overrides
# =============================================================================
...
_MODEL_STYLE = {
    "lr": {"color": "black", "linestyle": "-", "marker": "o"},
    "lgbm": {"color": "0.45", "linestyle": "--", "marker": "s"},
}
...
def plot_roc_panels(primary_results: dict[str, dict[str, Any]]) -> plt.Figure:
    fig, axes = plt.subplots(2, 2, figsize=(mm_to_in(170), mm_to_in(160)))
    for ax, (task_key, title) in zip(axes.ravel(), panel_specs()):
        r = primary_results[task_key]
        y = r["y_temp"].to_numpy()
        for model_key in ["lr", "lgbm"]:
            ...
            ax.plot(fpr, tpr, color=st["color"], linestyle=st["linestyle"], label=f"{block['label']}: {value:.3f} ({low:.3f}–{high:.3f})")
        ax.plot([0, 1], [0, 1], color="0.65", linestyle=":", linewidth=0.9)
    ...
    fig.subplots_adjust(left=0.10, right=0.99, top=0.98, bottom=0.09, wspace=0.28, hspace=0.34)
    return fig
...
def plot_pr_panels(primary_results: dict[str, dict[str, Any]]) -> plt.Figure:
    ...
    for ax, (task_key, title) in zip(axes.ravel(), panel_specs()):
        ...
        for model_key in ["lr", "lgbm"]:
            ...
            low, high = block["ci"]["PR_AUC"]
            st = _MODEL_STYLE[model_key]
            ax.plot(recall, precision, color=st["color"], linestyle=st["linestyle"], label=f"{block['label']}: {value:.3f} ({low:.3f}–{high:.3f})")
        ax.axhline(prevalence, color="0.65", linestyle=":", linewidth=0.9, label=f"Event rate: {prevalence:.3f}")
        ax.set_title(title, loc="left", fontweight="bold", pad=4)
        ax.set_xlabel("Recall")
        ax.set_ylabel("Precision")
        ax.set_xlim(0, 1)
...
def plot_dca_panels(primary_results: dict[str, dict[str, Any]], config: AnalysisConfig) -> plt.Figure:
    thresholds = np.arange(
        config.dca_threshold_min,
        config.dca_threshold_max + config.dca_threshold_step / 2,
        config.dca_threshold_step,
    ...
    for ax, (task_key, title) in zip(axes.ravel(), panel_specs()):
        ...
        for model_key in ["lr", "lgbm"]:
            ...
            dca = decision_curve(y, block["temporal_prob"], thresholds)
            st = _MODEL_STYLE[model_key]
            ax.plot(dca["threshold"], dca["model"], color=st["color"], linestyle=st["linestyle"], label=block["label"])
            all_curves.append(dca["model"].to_numpy())
        reference = decision_curve(y, r["models"]["lr"]["temporal_prob"], thresholds)
        ax.plot(reference["threshold"], reference["treat_all"], color="0.60", linestyle="-.", label="Treat all")
        ax.plot(reference["threshold"], reference["treat_none"], color="0.75", linestyle=":", label="Treat none")
        y_values = np.concatenate(all_curves + [reference["treat_all"].to_numpy(), np.zeros(len(reference))])
        finite = y_values[np.isfinite(y_values)]
        low = min(-0.05, float(np.quantile(finite, 0.02)) - 0.02)
        high = max(0.05, float(np.quantile(finite, 0.98)) + 0.02)
        ax.set_ylim(low, high)
        ax.set_title(title, loc="left", fontweight="bold", pad=4)
        ax.set_xlabel("Threshold probability")
        ax.set_ylabel("Net benefit")
        ax.set_xlim(config.dca_threshold_min, config.dca_threshold_max)
        ax.set_xticks(np.arange(0.1, config.dca_threshold_max + 0.001, 0.1))
        ax.tick_params(direction="out", length=2.5, width=0.6)
        ax.legend(frameon=False, loc="best", handlelength=2.5)
    fig.subplots_adjust(left=0.11, right=0.99, top=0.98, bottom=0.09, wspace=0.30, hspace=0.34)
    return fig


def plot_day7_matrix(matrix: pd.DataFrame, summary: pd.DataFrame, model_label: str) -> plt.Figure:
    fig, ax = plt.subplots(figsize=(mm_to_in(170), mm_to_in(155)))
    patterns = [
        ("Neither outcome", "o", "white", "0.45"),
        ("Non-home discharge only", "^", "0.75", "0.25"),
        ("Prolonged hospitalization only", "s", "0.45", "0.15"),
        ("Both outcomes", "D", "black", "black"),
    ]
    for pattern, marker, face, edge in patterns:
        sub = matrix.loc[matrix["observed_pattern"].eq(pattern)]
        ax.scatter(
            sub["p_los21"],
            sub["p_nonhome"],
    ...
    th_x = float(summary["threshold_los21"].iloc[0])
    th_y = float(summary["threshold_nonhome"].iloc[0])
    ax.axvline(th_x, color="black", linestyle="--", linewidth=0.9, zorder=3)
    ax.axhline(th_y, color="black", linestyle="--", linewidth=0.9, zorder=3)
    ax.set_xlabel("Predicted probability of hospital stay >21 days")
    ax.set_ylabel("Predicted probability of no direct home discharge")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    _format_probability_axes(ax)

    annotations = {
        "Lower risk for both outcomes": (th_x / 2, th_y / 2, "Lower / Lower"),
        "Higher non-home risk only": (th_x / 2, (th_y + 1) / 2, "Higher non-home risk"),
        "Higher prolonged-hospitalization risk only": ((th_x + 1) / 2, th_y / 2, "Higher prolonged-LOS risk"),
        "Higher risk for both outcomes": ((th_x + 1) / 2, (th_y + 1) / 2, "Higher / Higher"),
    }
    for _, row in summary.iterrows():
        x, y, short = annotations[row["matrix_group"]]
        ax.text(
            x,
            y,
            f"{short}\nn = {int(row['n'])} ({100*row['cohort_percent']:.1f}%)\n"
            f"Observed non-home: {100*row['observed_nonhome_rate']:.1f}%\n"
            f"Observed >21 d: {100*row['observed_los21_rate']:.1f}%",
            ha="center",
            va="center",
            fontsize=7.1,
            bbox={"facecolor": "white", "edgecolor": "0.25", "alpha": 0.88, "pad": 3.0},
            zorder=4,
        )
    ax.text(0.01, 1.02, model_label, transform=ax.transAxes, fontweight="bold", ha="left", fontsize=8)
    ax.legend(frameon=False, loc="lower center", bbox_to_anchor=(0.5, -0.24), ncol=2, handletextpad=0.5, columnspacing=1.0)
    fig.subplots_adjust(left=0.12, right=0.98, top=0.94, bottom=0.23)
    return fig


def plot_lr_coefficients(coefficients: pd.DataFrame, top_n: int = 10) -> plt.Figure:
    tasks = [key for key, _ in panel_specs()]
    titles = dict(panel_specs())
    fig, axes = plt.subplots(2, 2, figsize=(mm_to_in(170), mm_to_in(185)))
    for ax, task in zip(axes.ravel(), tasks):
        sub = coefficients.loc[
            coefficients["Task"].eq(task) & ~coefficients["Raw_variable"].eq("(Intercept)")
        ].nlargest(top_n, "Absolute_coefficient")
        sub = sub.sort_values("Coefficient")
        ax.barh(sub["Variable"], sub["Coefficient"], color="0.45", edgecolor="black", linewidth=0.35)
        ax.axvline(0, color="black", linewidth=0.7)
        ax.set_title(titles[task], loc="left", fontweight="bold", pad=4)
        ax.set_xlabel("Penalized logistic regression coefficient")
        ax.tick_params(axis="y", labelsize=6.4)
    fig.subplots_adjust(left=0.27, right=0.99, top=0.98, bottom=0.08, wspace=0.65, hspace=0.36)
    return fig
...
def plot_dca_panels_common_scale(
    ...
    for task_key, _ in panel_specs():
        ...
        for model_key in ["lr", "lgbm"]:
            dca = decision_curve(y, r["models"][model_key]["temporal_prob"], thresholds)
    ...
    global_low = max(-0.10, min(-0.03, float(np.quantile(values, 0.02)) - 0.02))
    global_high = min(0.70, max(0.05, float(np.quantile(values, 0.98)) + 0.02))
...
def plot_day7_matrix_summary(
    summary: pd.DataFrame,
    model_label: str,
) -> plt.Figure:
    """Render the prespecified four-stratum operational matrix without patient-level points."""
    required_groups = {
        "Lower risk for both outcomes",
        "Higher non-home risk only",
        "Higher prolonged-hospitalization risk only",
    ...
    if set(summary["matrix_group"].dropna()) != required_groups:
        raise ValueError("Day 7 matrix summary does not contain the four prespecified strata")
    ...
    fig, ax = plt.subplots(figsize=(mm_to_in(170), mm_to_in(112)))
    ax.set_xlim(0, 2)
    ax.set_ylim(0, 2)
    ax.set_aspect("equal")

    cells = [
        ("Lower risk for both outcomes", 0, 0, "Lower risk for both", "1.00"),
        ("Higher prolonged-hospitalization risk only", 1, 0, "Higher prolonged-LOS\nrisk only", "0.90"),
        ("Higher non-home risk only", 0, 1, "Higher non-home\nrisk only", "0.82"),
        ("Higher risk for both outcomes", 1, 1, "Higher risk for both", "0.68"),
    ]
    for group, x, y, short_title, shade in cells:
    ...
    ax.set_xlabel("Predicted risk of hospital stay >21 days", labelpad=7)
    ax.set_ylabel("Predicted risk of no direct home discharge", labelpad=10)
    ax.tick_params(length=0, pad=6)
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.text(
        0.0, 2.06, model_label,
        ha="left", va="bottom",
...
Receiver operating characteristic curves for no direct home discharge and hospital length of stay greater than 21 days at the Day 3 and Day 7 landmarks. Values in the legends are areas under the receiver operating characteristic curve with 95% bootstrap confidence intervals.

Figure 3. Calibration in the temporal-evaluation cohorts
Observed outcome proportions are plotted against mean predicted probabilities in approximately 10 quantile groups. The diagonal line indicates perfect calibration. Calibration intercepts and slopes are shown in the panel legends.

Figure 4. Decision curve analysis in the temporal-evaluation cohorts
Net benefit is shown across threshold probabilities from 0.01 to 0.70 for penalized logistic regression, LightGBM, treat-all, and treat-none strategies. A common vertical scale is used across panels.

Figure 5. Day 7 two-dimensional operational risk matrix
Predicted probabilities of no direct home discharge and hospital length of stay greater than 21 days from the penalized logistic regression models are shown jointly. Dashed lines indicate development-derived thresholds targeting approximately 80% sensitivity. Quadrant annotations report cohort size and observed outcome rates. The matrix characterizes joint operational risk and is not a validated treatment or disposition rule.

Additional Figure S1. Precision-recall curves in temporal evaluation
Additional Figure S2. Day 7 operational risk matrix using LightGBM
Additional Figure S3. Penalized logistic regression coefficients
Additional Figures S4-S7. LightGBM SHAP summaries for the four primary prediction tasks
"""
...
- Day 7 two-dimensional matrix; penalized logistic regression is primary
...
def run_publication_stage(config: AnalysisConfig) -> dict[str, Any]:
    ...
    matrix_lr, matrix_summary_lr = build_day7_matrix(primary_results, "lr", config)
    matrix_lgbm, matrix_summary_lgbm = build_day7_matrix(primary_results, "lgbm", config)
    save_dataframe(matrix_lr, paths["predictions"] / "day7_operational_matrix_lr_patients.csv")
    save_dataframe(matrix_lgbm, paths["predictions"] / "day7_operational_matrix_lgbm_patients.csv")
    save_dataframe(matrix_summary_lr, paths["tables"] / "day7_operational_matrix_lr_summary.csv")
    save_dataframe(matrix_summary_lgbm, paths["tables"] / "day7_operational_matrix_lgbm_summary.csv")
    ...
    save_figure(
        plot_day7_matrix_summary(matrix_summary_lr, "Penalized logistic regression"),
        paths["figures"] / "Figure_5_Day7_operational_matrix_LR",
        config.figure_dpi,
    )
    save_figure(plot_pr_panels(primary_results), paths["figures"] / "Figure_S1_PR_curves", config.figure_dpi)
    save_figure(
        plot_day7_matrix_summary(matrix_summary_lgbm, "LightGBM"),
        paths["figures"] / "Figure_S2_Day7_operational_matrix_LightGBM",
        config.figure_dpi,
    )
    save_figure(
...
# %% [markdown]
# ## Execute main analysis step by step
#
# Run these cells in order. Each primary task is executed separately so that a native-library crash can be localized. The statistical specification is unchanged.

# %% 

# =============================================================================
# A. Preflight, data loading, and cohort audit only
# =============================================================================
ACTIVE_CONFIG = _clone_and_apply_mode(CONFIG)
np.random.seed(ACTIVE_CONFIG.seed)
PROJECT = setup_project_directories(ACTIVE_CONFIG)
MAIN_PATHS = PROJECT["main"]
LOGGER = setup_logger(MAIN_PATHS["root"] / "01_main_analysis.log")
configure_matplotlib(ACTIVE_CONFIG)

print("Python:", platform.python_version())
print("Mode:", ACTIVE_CONFIG.mode)
print("LightGBM n_jobs:", ACTIVE_CONFIG.lgbm_n_jobs)
print("CV n_jobs:", ACTIVE_CONFIG.cv_n_jobs)
print("Native thread caps:", {k: os.environ.get(k) for k in [
    "OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS", "NUMEXPR_NUM_THREADS"
]})
print("Loaded thread pools:")
for _info in threadpool_info():
    print({k: _info.get(k) for k in ["user_api", "internal_api", "num_threads", "prefix", "version"]})

LOGGER.info("Stage 01 preflight started | mode=%s", ACTIVE_CONFIG.mode)
with threadpool_limits(limits=1):
    DATA, AUDITS = load_and_prepare_data(ACTIVE_CONFIG, MAIN_PATHS, LOGGER)
PRIMARY_SPECS, _ = build_task_specs(ACTIVE_CONFIG)
PREDICTOR_DICTIONARY = make_predictor_dictionary()
save_dataframe(PREDICTOR_DICTIONARY, MAIN_PATHS["tables"] / "predictor_dictionary.csv")

print("\nCohort audit")
display(AUDITS["flow"])
print("\nOutcome audit")
display(AUDITS["outcome_audit"])
print("\nPrimary tasks:", [s.key for s in PRIMARY_SPECS])

# %% 

# =============================================================================
# B. Helper for one primary prediction task
# =============================================================================
PRIMARY_RESULTS = {}
_SPEC_LOOKUP = {s.key: s for s in PRIMARY_SPECS}

def run_primary_task_step(task_key: str):
    if task_key not in _SPEC_LOOKUP:
        raise KeyError(task_key)
    spec = _SPEC_LOOKUP[task_key]
    print("=" * 80)
    print("START:", spec.label)
    print("=" * 80)
    with threadpool_limits(limits=1):
        result = fit_prediction_task(
            DATA, spec, ACTIVE_CONFIG, MAIN_PATHS, LOGGER
        )
    PRIMARY_RESULTS[task_key] = result
    # Persist each completed task immediately.
    checkpoint = {
        "analysis_version": ANALYSIS_VERSION,
        "signature_sha256": signature_hash(ACTIVE_CONFIG),
        "task_key": task_key,
        "result": result,
    }
    checkpoint_path = MAIN_PATHS["bundles"] / f"checkpoint_{task_key}.joblib"
    joblib.dump(checkpoint, checkpoint_path, compress=3)
    print("COMPLETED:", spec.label)
    print("Checkpoint:", checkpoint_path)
    for model_key in ["lr", "lgbm"]:
        b = result["models"][model_key]
        print(
            model_key,
            "AUROC=", round(b["point"]["AUROC"], 4),
            "PR-AUC=", round(b["point"]["PR_AUC"], 4),
            "Brier=", round(b["point"]["Brier"], 4),
        )
    gc.collect()
    return result

# %% [markdown]
# ### B1. Day 3 — no direct home discharge

# %% 
DAY3_NONHOME_RESULT = run_primary_task_step("day3_nonhome")

# %% [markdown]
# ### B2. Day 3 — hospital stay >21 days

# %% 
DAY3_LOS21_RESULT = run_primary_task_step("day3_los21")

# %% [markdown]
# ### B3. Day 7 — no direct home discharge

# %% 
DAY7_NONHOME_RESULT = run_primary_task_step("day7_nonhome")

# %% [markdown]
# ### B4. Day 7 — hospital stay >21 days

# %% 
DAY7_LOS21_RESULT = run_primary_task_step("day7_los21")

# %% [markdown]
# ### C. Finalize and lock Stage 01 outputs

# %% 

# =============================================================================
# C. Assemble primary results, tables, bundle, and manifest
# =============================================================================
_task_set_qa(PRIMARY_RESULTS, ACTIVE_CONFIG)
...
PUBLICATION_PERFORMANCE = make_publication_performance_table(PERFORMANCE)
OOF_PERFORMANCE = make_development_oof_performance(PRIMARY_RESULTS)
THRESHOLDS = make_threshold_table(PRIMARY_RESULTS, ACTIVE_CONFIG.target_sensitivity)
HYPERPARAMETERS = flatten_hyperparameters(PRIMARY_RESULTS)
FULL_PARAMETERS = extract_full_model_parameters(PRIMARY_RESULTS)
...
for _name, _table in {
    ...
    save_dataframe(_table, MAIN_PATHS["tables"] / f"{_name}.csv")
...
BUNDLE_PATH = _save_bundle(MAIN_BUNDLE, MAIN_PATHS, "primary_results_bundle")
MANIFEST_PATH = write_stage_manifest(
    "main",
    ACTIVE_CONFIG,
    MAIN_PATHS,
    extra={
        ...
        "N_day3": int(DATA["flag_day3_landmark"].sum()),
        "N_day7": int(DATA["flag_day7_landmark"].sum()),
        "primary_tasks": [s.key for s in PRIMARY_SPECS],
        "execution_mode": "stepwise_jupyter_threads1",
    },
)
...
display(PUBLICATION_PERFORMANCE)
MAIN_RESULTS = {
